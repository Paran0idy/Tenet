...
class NDArray:
    ...
    def permute(self, new_axes):
        ...
        new_stride = []
        for x in new_axes:
            new_shape.append(self._shape[x])
            new_stride.append(self._strides[x])
        return self.make(new_shape, tuple(new_stride), self._device, self._handle)
    ...
    def __getitem__(self, idxs):
        ...
        new_stride = []
        new_offset = 0
        for i, info in enumerate(idxs):
            new_shape.append((info.stop - info.start + info.step - 1) // info.step)
            new_stride.append(self._strides[i] * info.step)
            new_offset += info.start * self._strides[i]
            
        return self.make(new_shape, tuple(new_stride), self._device, self._handle, new_offset)
